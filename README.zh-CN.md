...
- **每个航班的详细笔记** — 每个航班都有自己的 📝 笔记页，记录统计装不下的细节：航班号、呼号、注册号、OUT/OFF/ON/IN 时刻（本地和 UTC）、滑行时间、V 速度、距离、载量与燃油、航路、SID/STAR、跑道、METAR、自由备注。所有字段都可选填；笔记绝不影响统计和 CSV。航班日志还能按有无笔记排序
...
点击 **Add Flight** 保存 — 或点 **📝 Add + Notes**，保存后立刻为这个航班写详细笔记（见下节）。

### 航班笔记（详细记录）

每个航班都可以携带自己的笔记 — 那些统计装不下的细节。入口在航班日志的 **Notes 列**：**📝** 表示已有笔记（点击查看），**+** 表示添加。点击 **Notes** 列头可把有笔记的航班排到最上面。

可以记录的内容（**全部选填** — 想写哪项写哪项）：

- **Flight info** — 航班号、呼号、注册号、飞行员
- **Times** — 出发/到达日期和 OUT / OFF / ON / IN 时刻（各有本地时间和 UTC），以及滑行出/入时间
- **Performance** — V1 / VR / V2 / VREF、飞行距离、接地下降率与着陆 G 值
- **Load & fuel** — 乘客、货物、装载燃油、消耗燃油
- **Route & procedures** — 完整航路、SID / STAR、使用跑道、METAR
- **Free notes** — 自由备注

贴心细节：只输入数字，单位（kt / nm / kg）会在显示时自动补上；时刻自动整理成 `HH:MM`；输入时会提示你以前用过的值。笔记与航班一起保存在你的设备上，不影响统计，也不会混进 CSV。备份请用 **Export → Full Backup (JSON)** — 恢复后航班和笔记依然关联。删除航班时其笔记也会一并删除（确认对话框会提醒你）。
