...
- **Fix a flight without losing anything** — edit a logged flight's date, route, aircraft, airline or duration from the pencil button in the Flight Log; its notes stay attached
...
- **Flight info** — flight number and callsign (suggested from your flight's airline: IATA prefix like `NH`, radio callsign like `JAPANAIR` — free typing still works, with a Heavy / Super picker), registration, pilot, departure / arrival terminal and gate, flight distance
...
- **Performance** — V1 / VR / V2 / VAPP / VREF, cost index, takeoff and landing configuration (one free-text line each, e.g. `Flaps 5 / FLEX 50`), cruise altitude and cruise Mach, touchdown rate, landing G, centerline offset, reverser credit
- **Load & fuel** — passengers, cargo, fuel on board, fuel used
- **Route & procedures** — full route, taxi routes (out and in), SID / STAR, runways, approach, alternate airport (suggests ICAO codes as you type)
- **Weather** — departure / arrival METARs, **decoded into plain English right below the raw report** (wind, visibility, clouds, temperature, pressure in hPa or inHg — decoded on your device), plus RVR
