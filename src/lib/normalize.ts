...
//
// 依存: airports.js（AP / IATA_TO_ICAO / CITY_*）・aircraft.js・airlines.js
import { AP, IATA_TO_ICAO, CITY_AIRPORT_TO_ICAO, CITY_TO_ICAO } from '../data/airports';
...
export function normalizeAirport(raw: unknown): string | null {
  if(raw == null) return null;
  const s = String(raw).toUpperCase().replace(/[^A-Z0-9]/g, '');
  if(!s) return null;
  if(ICAO_ALIASES[s]) return ICAO_ALIASES[s];
  if(s.length === 4 && AP[s]) return s;
  if(s.length === 3 && IATA_TO_ICAO[s]){
    return IATA_TO_ICAO[s];
  }
  if(CITY_AIRPORT_TO_ICAO[s]){
...
export function normalizeAirline(raw: unknown): string | null {
  if(raw == null) return null;
...
export function normalizeAircraft(raw: unknown): string | null {
  if(raw == null) return null;
